  ...
  try {
    ...
    if (error) {
      ...
      if (parsed.tableNotFound || parsed.htmlResponse) {
        console.error("[api/inventory] Inventory table not found. Run schema migration.");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!variantId || change === undefined || !reason) {
      return NextResponse.json({ error: "variantId, change, and reason are required" }, { status: 400 });
    }
    ...
    if (updateErr) return NextResponse.json({ error: getResponseError(updateErr).cleanedMessage }, { status: 500 });

    try {
      const logEntry: Record<string, unknown> = { variant_id: variantId, change, reason };
      if (orderId) logEntry.order_id = orderId;
      await admin.from("inventory_logs").insert(logEntry);
    } catch {
